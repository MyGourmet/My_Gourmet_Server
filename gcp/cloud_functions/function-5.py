...
def save_to_cloud_storage(content, filename, bucket, user_id, class_name):
...
def handler(req: https_fn.CallableRequest):
    try:
        ...
        blob_model = model_bucket.blob("gourmet_cnn_resnet_final_v2.tflite")
            ...
            for photo in photos_data["mediaItems"]:
                ...
                if content and classes[predicted] in [
                    ...
                    save_to_cloud_storage(
                        content, photo["filename"], bucket, userId, classes[predicted]
                    )
        ...
        #  access_token と一致する　userId の ドキュメントを探す
